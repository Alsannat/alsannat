...
class Bulk extends \Magento\Backend\App\Action
{
    ...
    {   
        ...
        $orderIds = $this->getRequest()->getParam('selectedOrders');
        $list_err = $list_success = [];
        if(count($orderIds) > 0){
            foreach ($orderIds as $key => $orderIncrement){
                $order = $this->modelOrder->loadByIncrementId((int) $orderIncrement);
                ...
                foreach ($tracksCollection->getItems() as $track) {
                    if($track->getCarrierCode() == 'smsashipping'){
                        $addTrack = true;
                    }
                }
                ...
                if(!$addTrack){
                    try {
                        $addSMSA = $this->addShipmentMPS($order);
                        if($addSMSA['status']){
                            $awd_number = $addSMSA['awb_number'];
                            $this->createShipment($order , $awd_number);
                            array_push($list_success, $order->getIncrementId());
                        }else{
                            array_push($list_err, $order->getIncrementId());
                        }
                    ...
                    $log_data = [
                        ...
                        'response' => 'SMSA shipment already exists',
                        'awd_status' => 'error'
                    ];
            ...
            if (count($list_success) > 0){
                $this->_messageManager->addSuccess('Add SMSA Shipment Succesfully for '.implode(",", $list_success));
            }

            if (count($list_err) > 0){
                $this->_messageManager->addError('Cannot create shipment SMSA for the order(s) '. implode(",", $list_err) .'. Read SMSA Shipping Logs for more information.');
            }
    ...
    public function addShipmentMPS($order)
    {
        ...
        $data = [
            ...
            'cntry' => (string) $address->getCountryId() ,
            'cCity' => (string) $city ,
