...
class Configurable
{
    public function afterGetJsonConfig(\Magento\Swatches\Block\Product\Renderer\Configurable $subject, $result) 
    {
        ...
        foreach ($subject->getAllowProducts() as $simpleProduct) {
            ...
            $jsonResult['sizes'][$simpleProduct->getId()] = $product->getAttributeText('size_new');
            ...
            $jsonResult['set_dimensions'][$simpleProduct->getId()] = $this->getSetDimensionHtml($product,$product->getHasMultipleDimensions());
    ...
    public function getSetDimensionHtml($product,$usesMultiple)
    {
        ...
        if(!is_null($product))
        {
            if($usesMultiple)
            {
                ...
                if(!is_null($product->getAttributeText('dimension_title_7'))&&!is_null($product->getData('dimension_height_7'))&&!is_null($product->getData('dimension_width_7'))&&!is_null($product->getData('dimension_length_7')))
                {
                    $html.="<strong>".$product->getAttributeText('dimension_title_7')." : H: ".$product->getData('dimension_height_7')." cm, D: ".$product->getData('dimension_length_7')." cm, W: ".$product->getData('dimension_width_7')." cm</strong><br/>";
                }
