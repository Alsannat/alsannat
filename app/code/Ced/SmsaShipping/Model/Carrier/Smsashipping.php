...
$dtd = '';
    ...
    foreach($this->cart->getItems() as $item){
        ...
        $product_set = $product->getData('product_set');
        
//         $dtd .= 'ID: '.$item->getProductId().'<br />';
//         $dtd .= 'Name: '.$item->getName().'<br />';
//         $dtd .= 'Set: '. $product_set.'<br />';
//         $dtd .= 'Sku: '.$item->getSku().'<br />';
//         $dtd .= 'Quantity: '.$item->getQty().'<br />';
//         $dtd .= 'Price: '.$item->getPrice().'<br />';
//         $dtd .= "<br />\n\n\n";            
     
        if ($product_set === '67' && $item->getPrice() != "0.0000"){ // && $item->getProduct()->getTypeId() != "configurable") {
                $has_large = true;                
        }
        else if($product_set != '67' && $item->getPrice() != "0.0000"){ // && $item->getProduct()->getTypeId() != "configurable") {
              $has_small = true;
        }
    }

//mail("elsaeedm@gmail.com", "alsannat", $dtd);
...
//            if ($item->getProduct()->getTypeId() != "configurable") {
            if ( $item->getPrice() != "0.0000") {
                if ($product_set === '67') {
                    $rate += (int)$item->getQty() * 30;
                } else {
                    if(!$has_large && $small_added == false ){
                        $rate += 20;//(int)$item->getQty() * 20;
                        $small_added = true;
                    }
                }
            }
        ...
        $_cart = $_objectManager->get('\Magento\Checkout\Model\Cart'); 
        $_subTotal = $_cart->getQuote()->getSubtotal();
