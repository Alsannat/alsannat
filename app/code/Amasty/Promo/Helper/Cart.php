...
class Cart
{
    ...
    public function addProduct(
        ...
        $image_url = $imageHelper->init($product, 'product_page_image_small')->setImageFile($product->getFile())->resize(50, 50)->getUrl();
        ...
        try {
            ...
            if (!$quote->hasData('is_copy')) {
                $this->promoMessagesHelper->showMessage(
                    __(
                        '<strong>%1</strong><span class="hide">was added to your shopping cart</span><span class="hide notice_img" style="background:url(%2);"></span>',
                        $product->getName(),
                        $image_url
                    ),
                    false,
