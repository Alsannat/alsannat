    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($type == 'item') {
            ...
            if ($itemData->getData('price')) {
                ...
                $itemData->setData('base_discount_amount', $this->helper->currency($item->getData('base_discount_amount'), $baseCurrencyCode));
                $itemData->setData('discount_amount_and_percent', (0 - $orderItem->getData('discount_percent')).'% '.$this->helper->currency($item->getData('discount_amount'), $orderCurrencyCode) );
            }
            ...
            $orderData->setData('subtotal_after_discount', $this->helper->currency($order->getData('subtotal_incl_tax') + $order->getData('discount_amount'), $orderCurrencyCode));
			...
			$invoiceData->setData('has_discount', $order->getDiscountAmount() < 0 ? 1 : 0);
            $invoiceData->setData('subtotal_after_discount', $this->helper->currency($order->getData('subtotal_incl_tax') + $order->getData('discount_amount'), $orderCurrencyCode));
