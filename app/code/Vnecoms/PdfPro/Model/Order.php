      ...
      //  var_dump($sourceData['customer']);die();
        ...
        */
        foreach ($source->getAllItems() as $item) {
            ...
            $itemModel = new \Vnecoms\PdfPro\Model\Order\Item(['item' => $item]);
        ...
        */
        ...
        if ($methodCode == "cashondelivery") {
           try {
                ...
                $paymentFeeRepository = $objectManager->get('Amasty\CashOnDelivery\Api\PaymentFeeRepositoryInterface');
                $quoteId = $order->getQuoteId();
                $paymentFee = $paymentFeeRepository->getByQuoteId($quoteId);

                if ($paymentFee) {
                    $sourceData['cod_fee'] = $this->helper->currency($paymentFee->getAmount(), $orderCurrencyCode);
                }
            } catch (Exception $e) {
            ...
            $zero = "0.00";
            $sourceData['cod_fee'] = $this->helper->currency($zero, $orderCurrencyCode);
        }
