    ...
    public function initCreditmemoData(\Magento\Sales\Model\Order\Creditmemo $creditmemo)
    {
    	...
    	*/
        foreach ($creditmemo->getAllItems() as $item) {
            ...
            $itemModel = new \Vnecoms\PdfPro\Model\Order\Creditmemo\Item(['item' => $item]);
            ...
            if ($total->canDisplay()) {
                ...
                foreach ($total->getTotalsForDisplay() as $totalData) {
                    ...
                    if($store_code == 'sa'){
                        ...
                        if( $totalData['label'] == 'Adjustment Fee:'){
                            $label = 'رسوم التعديل:';
                        }
                        ...
                        if( $totalData['label'] == 'Grand Total:'){
                            $label = 'يشمل المبلغ الإجمالي ضريبة القيمة المضافة:';
                        }
                    }
                    $totalArr[$area][] = new \Magento\Framework\DataObject(array('label' => $label, 'value' => $totalData['amount']));
                }
            }
        }
