...
class Filter extends \Magento\Email\Model\Template\Filter
{
    ...
    public function __construct(
        ...
        \Magento\Variable\Model\Source\Variables $configVariables, $variables = [])
    {
        $this->helper = $helper;
        $this->layout = $layout;
        $this->debug = $debug;
...
//                $expression = str_replace($variable, "\$this->templateVars[$variable]", $expression);
//            }
//            try {
//                eval($expression.';');
//            } catch (\Exception $e) {
...
//    }
    ...
    public function widgetDirective($construction)
    {
      ...
      //  $block = \Magento\Framework\App\ObjectManager::getInstance()
           // ->create('\VnEcoms\AdvancedPdfProcessor\Block\Filter\Widget',[]);die();
      //  $block = $this->_layout->createBlock('VnEcoms\AdvancedPdfProcessor\Block\Filter\Widget');
        //$block->setData(array('object'=>$obj, 'value' => $value,'type'=>$type, 'column'=>$column_setting, 'items'=>$items, 'totals'=>$totals))->setArea('adminhtml')->setIsSecureMode(true)->setTemplate('VnEcoms_AdvancedPdfProcessor::filter/widget.phtml');

        /* @var $layout \Magento\Framework\View\Layout */
       // $layout = \Magento\Framework\App\ObjectManager::getInstance()->create(
    ...
    //    );
     ...
     //   die();
      //  echo $this->widgetBlock->toHtml();die();
