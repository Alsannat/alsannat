define([
    ...
    return Element.extend({
        ...
        parseContent: function(){
    		...
    		var matches = [];
    		while(true){
    			var result = this.REGEX.exec(this.editor.value());
    			if(!result) break;
    			matches.push(result)
    		}
        ...
        addSection: function(sectionData, elementType, uniqueId=false){
        	...
        	var editable = elementType == this.ELEMENT_TYPE_SECTION_LAYOUT;
        	for(var fieldId in sectionData.fields){
        		var field 		= sectionData.fields[fieldId];
        		var fieldData 	= JSON.parse(JSON.stringify(this.fieldsData[field.type]));
        		...
        		fieldData['name'] 			= sectionName+'_'+field.id;
        		fieldData['displayArea'] 	= field.id;
        		fieldData['position'] 		= fieldId;
        		fieldData['parent'] 		= this.name+'.'+sectionName;
        		fieldData['editable'] 		= editable;
        		fieldData['pagebuilder'] 	= this;
        		fieldData['fieldPrefix'] 	= sectionId;
        		
        		elementData.children.push(fieldData);
        	}
        ...
        getSectionType: function(){
        	...
        	this.sectionTypes.each(function(sectionType){
        		if(self.pbResource.indexOf(sectionType.resource) >=0){
        ...
        editSection: function(section){
        	...
        	elements.each(function(element){
        		if(element.editor){
        			// element.editor.turnOff();
        			// element.editor.turnOn();
        		}else{
        			if(element.hasEditor) element.initEditor();
        		}
        	});
        ...
        updateContent: function(){
        	...
        	setTimeout(function(){
        		if(self.editor){
					if (window[self.editor.editor_obj_name]) {
						//window[self.editor.editor_obj_name].turnOff(); /*Make sure the editor is disable before set value*/
					}
