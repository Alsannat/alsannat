...
function getRegionCities(value,main_id) {
    if(!ajaxLoading) {
        ...
        jQuery.ajax({
            ...
            var options = '<select onchange="getCityState(this.value,\''+main_id+'\')" id="'+city_id+'-select" class="select" title="City" name="city-select" ><option value="">Please select a city</option>';
               ...
               // Render City
                for (var i = 0; i < responsecity.length; i++) {
                    ...
                    if (city >= 0) {
                       ...
                       options += '<option value="' + responsecity[i].city_code + '" disabled>' + responsecity[i].city_code + '</option>';
                    } else {
                        options += '<option value="' + responsecity[i].city_code + '">' + responsecity[i].city_code + '</option>';
                    }
                }
                options += "</select>";
